...
import { apiGetProduct, apiGetProducts, apiPostAddCart } from "@/api.js";
export default {
  components: {
    Navbar,
    Footer,
  ...
  setup() {
    ...
    const getRelatedProducts = (nowClass) => {
      ...
      apiGetProducts()
        .then((res) => {
          res.data.data.filter((item) => {
            if (
              item.category === nowClass.category &&
              item.title !== nowClass.title
            ) {
    ...
    const getProduct = (id) => {
      ...
      apiGetProduct(id)
        .then((res) => {
          ...
          getRelatedProducts(res.data.data);
